...
`/admin/analytics.html`（owner/admin限定）で、公開チャットのアクセス・利用状況を確認できます。トラフィック（セッション数・ランディング閲覧数）、来訪者数（ユニーク・新規・再訪、DAU/WAU/MAU）、平均セッション時間、デバイス/ブラウザ/OS/国地域別の内訳、よく使われる機能、お立場別の利用状況、来訪者ごとの初回訪問・最終アクティブ・セッション数の一覧（検索・お立場・状態でフィルタ可能）を、今日/7日間/30日間/3ヶ月/1年間/カスタム期間で切り替えて見られます。

仕組み:
- 公開チャットには元々ログイン・会員登録がなく、セッションIDも無操作30分または明示的な「入口にもどる」でリセットされるため、新規/再訪の判定ができませんでした。これを解決するため、`amasas_visitor`という匿名の永続ID（個人情報を含まないランダムUUID、`localStorage`に保存・ログアウトでも消さない）を追加しています。
- メッセージ数・フィードバック数など既存のログで分かることは重複して記録せず、そこから存在しなかった2つのシグナル（`landing_view`＝使い方画面の閲覧、`chat_started`＝チャット開始）だけを`api/track.js`経由で新たに記録します（`amasas_analytics_events`テーブル）。チャット本体の応答速度には影響しません（別系統の非同期呼び出し）。
- チャット内容・氏名などの個人情報・パスワード等は一切記録しません。記録するのはイベント種別・デバイス種別・ブラウザ・OS・国地域（Vercelの位置情報ヘッダーから取得、任意）・リファラーのみです。
